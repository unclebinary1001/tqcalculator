...
test("calculate correct F/G factor and future value", () => {
  render(
    <MantineProvider theme={theme}>
      <BrowserRouter>
        <Routes>
          <Route path="/" element={<Fg />} />
        </Routes>
      </BrowserRouter>
    </MantineProvider>
  );
  const amount = screen.getByLabelText("Gradient G");
  ...
  expect(factor).toBe("3.0500");
  expect(futureValue).toBe("$3050.00");
});
