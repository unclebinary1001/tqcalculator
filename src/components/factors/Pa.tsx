import {
  ...
  export default function Pa() {
    ...
    const calculateAns = () => {
      const { A, i, n } = form.values;
      const rate = i/100;
      const pa = (Math.pow(1 + rate,n) - 1)/(rate * (Math.pow(1 + rate,n)));
      const amount = A * pa;
      setAns({ factor: formatDecimals(pa, 4), presentValue: (A === 0)? "0": formatDecimals(amount, 2) });
    }
  
    const resetAll = () => {
      form.setValues({ A: 0, i: 0, n: 0 });
